...
import {
  ...
  getOilConsumptionPerMonth,
  QueryViewFileAnalysisOil,
} from './components/oils/consumption-equipment/consumption-equipment.service';
...
export class AppService {
  ...
  public ListConsumptionLubricantPerMonth(userid: number, startDate: string, endDate: string) {
    return DummyPromise()
      .then(result => {
        return this._UsersService.Gets(<UserEntity>{
          id: userid,
          role: 'BUQUE',
        });
      })
  ...
  public SaveFileAnalysisOil(buqueId: number, ETM_OilAnalysis_Oid: string): Promise<QueryViewFileAnalysisOil[]> {
    return DummyPromise()
      .then(result => {
        return this._ConsumptionEquipmentService.ViewFileAnalysisOil(buqueId, ETM_OilAnalysis_Oid);
      })
      .then((result: QueryViewFileAnalysisOil[]) => {
        return result;
      })
      .catch(result => {
        return [];
  ...
  public ViewFileAnalysisOil(buqueId: number, ETM_OilAnalysis_Oid: string): Promise<QueryViewFileAnalysisOil[]> {
    return DummyPromise()
      .then(result => {
        return this._ConsumptionEquipmentService.ViewFileAnalysisOil(buqueId, ETM_OilAnalysis_Oid);
      })
  ...
  public async ConsumptionLubricantPerMonthPerListUsers(users: UserEntity[], startDate: string, endDate: string) {
    ...
    for await (const itemUser of users) {
      ...
      DashboardListMonthLubricant.getOilConsumptionPerMonth = await this._ConsumptionEquipmentService.getOilConsumptionPerMonth(
        itemUser.id,
        startDate,
        endDate,
      );
