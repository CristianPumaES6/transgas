...
export class OilsController {
  ...
  GetsDataServer(@Headers() headers, @Query() oilEntity: OilEntity): Promise<any> {
    ...
    return DummyPromise()
      ...
      .then((GroupsOilEntity: GroupOilEntity[]) => {
      ...
      .then((EquipmentSystemsEntity: EquipmentSystemEntity[]) => {
      ...
      .then((ConsumptionsEquipmentEntity: ConsumptionEquipmentEntity[]) => {
      ...
      .then((bunkersOilEntity: BunkerOil[]) => {
      ...
      .then((equipmentOilCompatibilityEntity: EquipmentOilCompatibilityEntity[]) => {
  ...
  async SaveDataLubricante(@Headers() headers, @Body() saveDateOils: DataModuleOils): Promise<any> {
    ...
    return DummyPromise()
      ...
      .then((resultMappingGroupOils: Mapping[]) => {
      ...
      .then((resultMappingEquipmentSystem: Mapping[]) => {
      ...
      .then((resultMappingEquipmentOilCompatibility: Mapping[]) => {
      ...
      .then((resultConsumptionEquipment: SaveListConsumptionEquipmentEntity) => {
      ...
      .then((listaDeConsumosRegistrados: DailyOilConsumptionData[]) => {
        // tenemos que enviar un correo lo enviamos
        if (listaDeConsumosRegistrados && listaDeConsumosRegistrados.length > 0) {
          ...
          return SendMailHTMLOverCosumption(
            'cristian.puma.es6@gmail.com;',
            headerToken.name,
            FormatDateUTCToDate(GetDate()),
            listaDeConsumosRegistrados,
          );
        } else {
          return true;
        }
      })
      .then((resultSendMail: boolean) => {
        // retornamos una Respuesta exitosa.
        return {
          status: HttpStatus.OK,
          message: 'OK',
